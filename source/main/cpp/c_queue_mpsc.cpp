...
namespace ncore
{
    ...
    namespace mpsc
    {
        ...
        struct queue_t
        {
            ...
            void push(u32 producer_index, u64 item)
            {
                spsc_queue_t* queue = m_producer_queues[producer_index];
                queue_enqueue(queue, item);
                m_queues_state.fetch_or(1 << producer_index, std::memory_order_release);
            }
    ...
    bool queue_enqueue(mpsc_queue_t* _queue, u32 producer_index, u64 item)
    {
        mpsc::queue_t* queue = (mpsc::queue_t*)_queue;
        queue->push(producer_index, item);
        return true;
    }
